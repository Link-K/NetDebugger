...
function ProgrammerCalculator({ onClose }: { onClose: () => void }) {
	...
	const parseBigInt = (raw: string, radix: 2 | 8 | 10 | 16): bigint => {
		...
		const pref =
			radix === 16 ? "0x" : radix === 10 ? "" : radix === 2 ? "0b" : "0o";
	...
	const computeBinary = (
		op: NonNullable<typeof pendingOp>,
		left: bigint,
		right: bigint,
	) => {
		switch (op) {
	...
	return (
		<div className="programmer-calc new-layout">
			...
			<button
				className="copy-top"
				onClick={() =>
					navigator.clipboard?.writeText(formatBigInt(value, base))
				}
			>
				Copy
			</button>
			...
			<div className="pc-grid">
				...
				<div className="pc-bits-wrap below-pending">
					...
					<div className="pc-bit-labels pc-bit-labels-inline">
						{(() => {
							...
							for (let i = 15; i >= 0; i--) {
								labels.push(
									<span key={i} className="pc-bit-label">
										{[12, 8, 4, 0].includes(i) ? String(i) : ""}
									</span>,
								);
							}
							return labels;
				...
				<div className="kp-main-grid">
					<div className="grid-row">
						{["7", "8", "9"].map((k) => (
							<button key={k} onClick={() => append(activeTarget(), k)}>
								{k}
							</button>
						))}
					...
					<div className="grid-row">
						<button
							className="kp-zero"
							onClick={() => append(activeTarget(), "0")}
						>
							0
						</button>
						<button onClick={() => backspace(activeTarget())}>←</button>
						<button className="kp-eq" onClick={equals}>
							=
						</button>
						<button onClick={() => setOp("+")}>+</button>
					</div>
...
function UDPServerView({ active }: { active: boolean }) {
	...
	useEffect(() => {
		(async () => {
			try {
				const ips = (await invoke<string[]>("list_local_ips")) ?? [];
				setDetectedIps(ips);
				if ((ip === "0.0.0.0" || ip === "")) {
					const pref = ips.find((p) => /^10\.|^192\.168\.|^172\.(1[6-9]|2[0-9]|3[0-1])/.test(p)) || ips[0];
					if (pref) setIp(pref);
				}
			} catch (e) {
				// ignore
	...
	const addHistory = (key: string, value: string) => {
		...
		setHistories((prev) => {
			...
			const next = [cleaned, ...prevList.filter((v) => v !== cleaned)].slice(
				0,
				50,
			);
	...
	useEffect(() => {
		...
		listen("udp:message", (event) => {
			try {
				...
				const who = bind
					? `[${escapeHtml(bind)}] ${escapeHtml(from)}`
					: escapeHtml(from);
			...
			<div className="control-row send-row send-row-2">
				...
				<div className="send-group">
					<div
						...
						onClick={() =>
							setSendModeLocal((m) => (m === "ascii" ? "hex" : "ascii"))
						}
						style={{
							display: "inline-flex",
							alignItems: "center",
...
function UDPClientView({ active }: { active: boolean }) {
	...
	const sendOnce = async () => {
		try {
			...
			const b64 = btoa(String.fromCharCode(...bytes));
			await invoke<string>("udp_client_send_from", {
				bindAddr: currentBind,
				toAddr: sendTarget,
				dataB64: b64,
			});
		...
		<div className={"view" + (active ? " active" : " hidden")}>
			...
			<div className="toolbar-row" style={{ marginBottom: 4 }}>
				...
				<label>
					...
					<input
						value={port}
						onChange={(e) =>
							setPort(e.currentTarget.value.replace(/[^0-9]/g, ""))
						}
						onBlur={() => addHistory("bind_port", port)}
						style={{ width: 100 }}
					/>
				</label>
				<button
			...
			<div className="control-row send-row send-row-2">
				...
				<div className="send-group">
					<div
						role="switch"
						aria-checked={sendMode === "hex"}
						onClick={() =>
							setSendMode((m) => (m === "ascii" ? "hex" : "ascii"))
						}
						style={{
							display: "inline-flex",
							alignItems: "center",
						...
						{connections.map((c) => (
							...
							>
								...
								<button
									...
									onClick={async () => {
										try {
											const res = await invoke<string>("stop_udp_client", {
												bindAddr: c,
											});
											appendStatus(res ?? `stopped ${c}`);
											setConnections((prev) => prev.filter((v) => v !== c));
										} catch (e) {
											appendStatus(String(e));
										}
									}}
...
function TCPServerView({ active }: { active: boolean }) {
	...
	const sendOnce = async () => {
		try {
			...
			const b64 = btoa(String.fromCharCode(...bytes));
			await invoke<string>("tcp_server_send", {
				bindAddr: bindAddr,
			...
			<div className="control-row send-row send-row-1">
				<label className="field field-peer">
					...
					<input
						...
						placeholder="ip:port"
						value={toPeer ?? ""}
						onChange={(e) => setToPeer(e.currentTarget.value || null)}
					/>
					<datalist id="hist-peers">
						{clients.map((c) => (
					...
					<ul className="connections-list">
						{clients.map((c) => (
							<li
								key={c}
								className={
									c === toPeer ? "connection-item current" : "connection-item"
								}
								style={{
									display: "flex",
									alignItems: "center",
...
function TCPClientView({ active }: { active: boolean }) {
	...
	const toggleConnection = async () => {
		...
		if (!isConnected(addr)) {
			try {
				const res = await invoke<string>("start_tcp_client", {
					remoteAddr: addr,
				});
			...
			try {
				const res = await invoke<string>("stop_tcp_client", {
					remoteAddr: addr,
				});
	...
	const sendOnce = async () => {
		try {
			...
			const b64 = btoa(String.fromCharCode(...bytes));
			await invoke<string>("tcp_client_send", {
			...
			<div className="toolbar-row" style={{ marginBottom: 4 }}>
				...
				<label style={{ marginLeft: 12 }}>
					...
					<input
						...
						onBlur={() => addHistory("tcp_remote", remoteAddr)}
						style={{ width: 220 }}
					/>
					<datalist id="hist-tcp-remote">
						{(histories["tcp_remote"] ?? []).map((h) => (
							<option key={h} value={h} />
						))}
					</datalist>
				</label>
				...
				<button
					...
					style={{ marginLeft: 8 }}
				>
					{isConnected(remoteAddr.trim()) ? "断开" : "连接"}
				</button>
			</div>
								...
								<button
									...
									onClick={async () => {
										try {
											...
											appendStatus(res ?? `disconnected ${c}`);
											setConnections((prev) => prev.filter((v) => v !== c));
										} catch (e) {
											appendStatus(String(e));
										}
		...
		<div className="app-root">
			...
			>
				...
				>
					...
					<path
						d="M8.5 15.5h1.5v1.5H8.5zM11 15.5h1.5v1.5H11zM13.5 15.5h1.5v1.5h-1.5zM8.5 12h1.5v1.5H8.5zM11 12h1.5v1.5H11zM13.5 12h1.5v1.5h-1.5z"
						fill="currentColor"
					/>
				</svg>
			</button>
			...
			<CommandsSidebar
				...
				onApply={applyCommandToActiveView}
			/>
